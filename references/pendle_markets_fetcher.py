...
# ───────────── utilities ─────────────
RESET_MODE = False          # force full refresh
DRY_RUN    = False          # skip DB upload
...
def make_api_request(url, method="GET", headers=None, data=None,
    ...
    """HTTP request with 429 retry + throttle."""
    headers = headers or {}
...
def safe_get(dct, keys, default=None):
    """Nested dict getter that tolerates missing / None."""
    cur = dct
    for k in keys:
...
def get_existing_markets(**_):
    """Return a set of market addresses already stored."""
    if RESET_MODE:
        return set()
...
def get_chain_ids(**_):
    """Fetch supported chain IDs from Pendle."""
    return make_api_request(f"{BASE_URL}/chains").get("chainIds", [])

def fetch_markets(**context):
    """Download markets for every chain and save new ones to CSV."""
    ...
    pd.DataFrame(new_rows, columns=selected_cols).to_csv(csv_path, index=False)
    return csv_path
...
def upload_to_rds(**context):
    ...
    if DO_NOT_UPLOAD:
        max_show = 25                      # prevent gigantic log spam
...
with DAG(
    ...
    start_date=datetime(2025, 5, 17, 2, 30),
    catchup=False,
    max_active_runs=1,
) as dag:

    get_existing_markets = PythonOperator(
        task_id="get_existing_markets",
        python_callable=get_existing_markets,
    )

    get_chain_ids = PythonOperator(
    ...
    get_existing_markets >> get_chain_ids >> fetch_markets >> upload_to_rds
